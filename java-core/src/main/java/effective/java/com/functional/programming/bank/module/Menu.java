package effective.java.com.functional.programming.bank.module;
    ...
    private void runSelectedItem(int item, List<User> users){
        switch (item){
            case 1:
                ...
                break;
            case 2:
                printUsers(userService.getAdultUsersWithNotRepaidLoans(users));
                break;
            case 3:
                printUsers(userService.getAdultBoysWithRepaidLoans(users));
                break;
            case 4:
                userService.groupUsersByBankWithRepaidLoans(users).forEach((key, value) ->
                        System.out.println("Bank: " + key.toString() + "\nUsers: \n" + printUsers(value)));
                break;
            case 5:
                userService.groupRepaidLoansByBank(users).forEach((key, value) ->
                        System.out.println("Bank: " + key.toString() + "\nLoans: \n" + loansPrintData(value)));
                break;
            case 6:
                userService.getUniqueFemaleNamesWithRepaidLoans(users).forEach(System.out::println);
                break;
            case 7:
                userService.getAverageLoanAmountForUserName(users).forEach((key, value) ->
                        System.out.println(key + " - " + value));
                break;
            case 8:
                userService.getThreeBiggestLoansAmountGroupedByUserName(users).forEach((key, value) ->
